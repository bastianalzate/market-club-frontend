...
export default function BeerClubSection() {
  const Content = ({ align = 'left' }: { align?: 'left' | 'center' }) => (
    <div className={`relative z-10 ${align === 'center' ? 'text-center' : ''}`}>
      <div>
        <h3
          className={`text-white mb-4 lg:mb-6 ${align === 'center' ? 'mx-auto' : ''}`}
          style={{
        ...
        >
          Beneficios del club: descuentos, acceso exclusivo, caja sorpresa mensual.
        </p>

        <button
          className={`bg-black text-white px-6 py-3 lg:px-8 lg:py-4 rounded-lg font-bold uppercase tracking-wide flex items-center gap-3 hover:bg-gray-800 transition-colors duration-200 ${align === 'center' ? 'mx-auto justify-center' : ''}`}
